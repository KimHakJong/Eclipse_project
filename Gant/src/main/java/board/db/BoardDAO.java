...
import javax.sql.DataSource;
		...
		// 공지사항 게시물 갯수 구하기 ( 메인화면 용)
				public int getNoticeListCount() {
					  ...
					  int x = 0; //글의 갯수
						 try {
							conn = ds.getConnection();
							String sql = "select count(*) from boards where BOARD_NOTICE='true' ";
							// PreparedStatement 객체얻기
								pstmt = conn.prepareStatement(sql);
								rs = pstmt.executeQuery();
		...
		//공지사항 게시글 리스트 ( 메인화면 용)-> 오버로딩
				public List<Board> getBoardNoticeList(int page, int limit) {
					  Connection conn = null;
					  PreparedStatement pstmt = null;
					  ResultSet rs = null;
				 // page : 페이지
				 ...
				 // 	  
					  ...
					  String sql = "select * "
							    ...
							    + "              order by BOARD_RE_REF desc,"
							    + "              BOARD_RE_SEQ asc) j "                                      
							    + "       where rownum<=? and j.BOARD_NOTICE='true' "
							    + "        ) "
							    + " where rnum>=? and rnum<=? ";  
